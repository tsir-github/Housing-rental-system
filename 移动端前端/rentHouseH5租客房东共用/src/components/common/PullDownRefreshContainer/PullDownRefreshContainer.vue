...
async function request() {
  console.log("PullDownRefreshContainer request() 被调用，当前页码:", pageInfo.value.current);
  //   调用接口
  await props.request(pageInfo.value);
  //   请求成功后才递增页码
  pageInfo.value.current++;
  console.log("PullDownRefreshContainer request() 完成，页码递增为:", pageInfo.value.current);
}
// 全部加载外部，后续触底不需要再重新请求
