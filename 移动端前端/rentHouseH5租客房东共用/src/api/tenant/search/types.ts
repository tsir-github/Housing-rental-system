...
export interface RoomInterface {
  ...
  publisherType?: PublisherType; // 发布者类型
  publisherId?: number | string; // 发布者ID
  complianceScore?: number; // 合规性评分
...
export interface RoomDetailInterface {
  ...
  rent: number | string;
  apartmentId: number | string;
  publisherType?: number; // 发布者类型：1-官方，2-房东
  publisherId?: number | string; // 发布者ID
  isRelease: string | number;
  reviewStatus?: number; // 审核状态：1-待审核，2-审核通过，3-审核拒绝
  complianceScore?: number; // 合规性评分
  ...
  reviewDetail?: {
    ...
    photoQualityScore?: number; // 图片质量评分
    descriptionAccuracyScore?: number; // 描述准确性评分
    facilityCompletenessScore?: number; // 设施完整性评分
    locationAccuracyScore?: number; // 位置准确性评分
