  ...
  <el-card>
    ...
    >
      ...
      <el-form-item label="发布者类型" prop="publisherType">
        <el-radio-group v-model="formData.publisherType" class="ml-4">
          <el-radio :label="PublisherType.OFFICIAL">
            官方发布
          </el-radio>
          <el-radio :label="PublisherType.LANDLORD">
            房东发布
          </el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item 
        v-if="formData.publisherType === PublisherType.LANDLORD" 
        label="房东ID" 
        prop="publisherId"
      >
        <el-input-number 
...
const formData = ref<SaveRoomInterface>({
  ...
  publisherType: PublisherType.OFFICIAL, // 默认为官方发布
...
const rules = reactive({
  ...
  publisherType: [{ required: true, message: '请选择发布者类型', trigger: 'change' }],
  publisherId: [
    { 
      validator: (rule: any, value: any, callback: any) => {
        if (formData.value.publisherType === PublisherType.LANDLORD && !value) {
          callback(new Error('房东发布时必须输入房东ID'))
        } else {
          callback()
        }
      }, 
  ...
  complianceScore: [
    { 
      validator: (rule: any, value: any, callback: any) => {
        if (formData.value.publisherType === PublisherType.LANDLORD) {
          if (value === undefined || value === null || value === '') {
            callback(new Error('房东发布时必须输入合规评分'))
          } else if (value < 0 || value > 10) {
            callback(new Error('合规评分必须在0-10分之间'))
          } else {
            callback()
          }
        } else {
...
async function getRoomInfoByIdHandle(id: number | string) {
  try {
    ...
    // 设置发布者信息
    formData.value.publisherType = data.publisherType
    formData.value.publisherId = data.publisherId
    formData.value.complianceScore = data.complianceScore
