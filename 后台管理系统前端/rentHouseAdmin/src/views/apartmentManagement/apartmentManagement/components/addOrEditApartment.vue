...
// 重置市数据
function resetCity() {
  formData.value.cityId = null as any
  formData.value.cityName = ''
  areaInfo.cityList = []
}
// 重置区数据
function resetDistrict() {
  formData.value.districtId = null as any
...
const provinceChangeCallback = async () => {
  ...
  if (provinceId) {
    // 设置省份名称
    const selectedProvince = areaInfo.provinceList.find(item => item.id === provinceId)
    formData.value.provinceName = selectedProvince?.name || ''
...
const provinceClearCallback = () => {
  formData.value.provinceId = null as any
...
const cityChangeCallback = async () => {
  ...
  if (cityId) {
    // 设置城市名称
    const selectedCity = areaInfo.cityList.find(item => item.id === cityId)
    formData.value.cityName = selectedCity?.name || ''
...
const districtChangeCallback = async () => {
  ...
  if (districtId) {
    // 设置区域名称
    const selectedDistrict = areaInfo.districtList.find(item => item.id === districtId)
    formData.value.districtName = selectedDistrict?.name || ''
  }
}
// 区域清除回调
...
function remoteMethod(keywords: string) {
  if (keywords.trim()) {
    // 确保省市区都已选择
    if (!formData.value.provinceId || !formData.value.districtId) {
    ...
    if (!AMap.value) {
      ElMessage.error('地图服务未初始化，请刷新页面重试')
      return
    }
      ...
      try {
        ...
        autoComplete.search(keywords, function (status: any, result: any) {
          ...
          if (status === 'complete' && result?.tips) {
            ...
            console.warn('地址搜索失败:', status, result)
            addressDetailOptions.value = []
            if (status === 'error') {
              ElMessage.error('地址搜索服务异常，请稍后重试')
            }
          }
        })
      } catch (error) {
        console.error('地址搜索插件加载失败:', error)
        ElMessage.error('地址搜索功能异常，请检查网络连接')
        addressDetailOptions.value = []
      }
    })
  } else {
