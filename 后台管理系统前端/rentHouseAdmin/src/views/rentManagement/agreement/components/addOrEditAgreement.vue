...
import { AgreementSource, AgreementStatus, AppointmentStatus } from '@/enums/constEnums'
import {
  AgreementInterface,
  RoomInfoInterface,
...
const formData = ref<AgreementInterface>({
  ...
  creatorType: 1, // 官方创建
  creatorId: 1, // 默认官方人员ID为1
  applicationId: undefined, // 手动创建时为空
  autoGenerated: 0, // 手动创建 - 后端期望接收Integer类型：0-否，1-是 (对应数据库 auto_generated tinyint DEFAULT '0')
  additionalInfo: '',
})
// 表单验证规则
...
async function addOrUpdateAgreementInfoHandle() {
  try {
    ...
    // 如果是从预约页面跳转过来的，保存成功后更新预约状态
    if (route.query?.source === 'appointment' && route.query?.appointmentId) {
      try {
        await updateAppointmentStatusById(route.query.appointmentId as string, AppointmentStatus.VIEWED)
        ElMessage.success('租约保存成功，预约状态已更新为已看房')
      } catch (error) {
        console.log('更新预约状态失败:', error)
        ElMessage.success('租约保存成功，但预约状态更新失败')
      }
    } else {
...
onMounted(async () => {
  ...
  // 检查是否是从预约管理页面跳转过来的
  if (route.query?.source === 'appointment') {
    // 自动填充预约信息
    if (route.query.name) {
    ...
    if (route.query.apartmentId) {
      areaInfo.apartmentId = route.query.apartmentId as string
      formData.value.apartmentId = route.query.apartmentId as string
    }
    if (route.query.roomId) {
      areaInfo.roomId = route.query.roomId as string
      formData.value.roomId = route.query.roomId as string
    }
    
    // 设置租约来源为预约看房
    formData.value.sourceType = AgreementSource.NEW
    formData.value.autoGenerated = 0 // 手动创建 - 后端期望接收Integer类型：0-否，1-是
    
    // 如果有房间信息，需要获取对应的地区信息
    if (route.query.apartmentId) {
      try {
        // 这里需要根据apartmentId获取公寓信息，然后设置对应的省市区
        // 暂时先获取房间列表
        await getRoomListHandle(route.query.apartmentId as string)
      } catch (error) {
        console.log('获取房间信息失败:', error)
      }
    }
    
    ElMessage.success('已自动填充预约信息，请完善其他租约详情')
