...
import com.atguigu.lease.web.admin.vo.room.RoomRejectVo;
...
public class RoomController {
    ...
    @Operation(summary = "根据条件分页查询房间列表")//current`和`size`为分页相关参数，分别表示**当前所处页面**和**每个页面的记录数**。
    @GetMapping("pageItem")
    ...
    public Result updateReleaseStatusById(Long id, ReleaseStatus status) {
        service.updateReleaseStatus(id, status);
        return Result.ok();
    }
    ...
    @Operation(summary = "打回房间审核")
    @PostMapping("reject")
    public Result rejectRoom(@RequestBody RoomRejectVo roomRejectVo) {
        service.rejectRoom(roomRejectVo.getRoomId(), roomRejectVo.getRejectReason());
        return Result.ok();
    }
